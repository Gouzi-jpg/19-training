...
public class OrderService : IOrderService
{
    ...
    public async Task<ServiceResult<Order>> CreateOrderAsync(int customerId, IReadOnlyList<NewOrderLine> lines)
    {
        ...
        var linesError = ValidateLines(lines);
        ...
        foreach (var line in lines)
        {
            ...
            var itemError = ValidateOrderItem(product, line);
            ...
            // 折扣集中在 CalculateTotal / GetDiscountRate，建單只存原價 snapshot。
    ...
    private static string? ValidateOrderItem(Product? product, NewOrderLine line)
    {
    ...
    public async Task<ServiceResult<Order>> CancelOrderAsync(int id)
    {
        ...
        // 先還原庫存，再改狀態；外層已驗證訂單為 Pending/Confirmed。
